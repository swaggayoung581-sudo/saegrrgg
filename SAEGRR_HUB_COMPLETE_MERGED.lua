...
do
	...
	subSplash.TextColor3 = Color3.fromRGB(70,180,255)
...
BX.module("features.movement", function(BX)
    ...
    local K = {
        ...
        SPEED             = 1000,  -- outbound cruise ceiling (SAEGRR TP SPEED MAX)
    ...
    function M.setSpeed(value)
        value = math.clamp(tonumber(value) or K.SPEED, 40, 1000)
        K.SPEED = value
        return K.SPEED
    end
...
do
    ...
    local BLUE = Color3.fromRGB(55,175,255)
    local BLUE2 = Color3.fromRGB(20,90,180)
    local RED = Color3.fromRGB(45,110,180)
    local BG = Color3.fromRGB(7,12,22)
    local CARD = Color3.fromRGB(13,24,40)
    local WHITE = Color3.fromRGB(245,245,245)
    local MUTED = Color3.fromRGB(155,175,195)

    local function corner(o,r)
        local c=Instance.new("UICorner",o); c.CornerRadius=UDim.new(0,r or 8); return c
    end
    local function stroke(o,c,t)
        local st=Instance.new("UIStroke",o); st.Color=c; st.Thickness=t or 1.5; return st
    end

    -- Main panel: compact, screenshot-like proportions.
    local main = Instance.new("Frame")
    main.Name="Main"
    main.Size=UDim2.fromOffset(360,455)
    main.Position=UDim2.new(0.5,-180,0.5,-228)
    ...
    main.Parent=gui
    corner(main,12)
    local mainStroke=stroke(main,BLUE,2)
    local mainGlow=stroke(main,BLUE,8); mainGlow.Transparency=.72

    local top=Instance.new("Frame",main)
    top.Size=UDim2.new(1,0,0,48); top.BackgroundTransparency=1; top.Active=true

    local title=Instance.new("TextLabel",top)
    title.Size=UDim2.new(1,-105,0,24); title.Position=UDim2.fromOffset(14,7)
    title.BackgroundTransparency=1; title.Font=Enum.Font.GothamBold; title.Text="SAEGRR HUB"
    title.TextColor3=WHITE; title.TextSize=16; title.TextXAlignment=Enum.TextXAlignment.Left

    local made=Instance.new("TextLabel",top)
    made.Size=UDim2.new(1,-105,0,15); made.Position=UDim2.fromOffset(14,29)
    made.BackgroundTransparency=1; made.Font=Enum.Font.Gotham; made.Text="MADE BY SAEGRR"
    made.TextColor3=BLUE; made.TextSize=8; made.TextXAlignment=Enum.TextXAlignment.Left

    local close=Instance.new("TextButton",top)
    close.Size=UDim2.fromOffset(28,28); close.Position=UDim2.new(1,-38,0,8)
    close.BackgroundColor3=Color3.fromRGB(35,12,14); close.BorderSizePixel=0; close.Text="×"
    close.TextColor3=Color3.fromRGB(255,90,90); close.Font=Enum.Font.GothamBold; close.TextSize=18; corner(close,7)

    local lock=Instance.new("TextButton",top)
    lock.Size=UDim2.fromOffset(58,22); lock.Position=UDim2.new(1,-102,0,11)
    lock.BackgroundColor3=Color3.fromRGB(12,25,17); lock.BorderSizePixel=0; lock.Text="LOCK"
    lock.TextColor3=BLUE; lock.Font=Enum.Font.GothamBold; lock.TextSize=8; corner(lock,6)

    -- Header dragging works on mobile + PC.
    ...
    lock.MouseButton1Click:Connect(function()
        locked=not locked; lock.Text=locked and "LOCKED" or "LOCK"
        lock.TextColor3=locked and WHITE or BLUE
        main.Active=not locked
    end)
    close.MouseButton1Click:Connect(function() main.Visible=false end)

    local content=Instance.new("ScrollingFrame",main)
    content.Size=UDim2.new(1,-16,1,-58); content.Position=UDim2.fromOffset(8,52)
    content.BackgroundTransparency=1; content.BorderSizePixel=0; content.ScrollBarThickness=3
    content.ScrollBarImageColor3=BLUE; content.AutomaticCanvasSize=Enum.AutomaticSize.Y
    content.ScrollingDirection=Enum.ScrollingDirection.Y
    local list=Instance.new("UIListLayout",content); list.Padding=UDim.new(0,6); list.SortOrder=Enum.SortOrder.LayoutOrder
    local pad=Instance.new("UIPadding",content); pad.PaddingLeft=UDim.new(0,3); pad.PaddingRight=UDim.new(0,3); pad.PaddingBottom=UDim.new(0,10)

    local function label(text,h,size,col)
        local x=Instance.new("TextLabel",content); x.Size=UDim2.new(1,0,0,h or 18); x.BackgroundTransparency=1
        x.Font=Enum.Font.GothamBold; x.Text=text; x.TextSize=size or 10; x.TextColor3=col or WHITE
        x.TextXAlignment=Enum.TextXAlignment.Left; return x
    end
    local function btn(text,cb,h)
        local b=Instance.new("TextButton",content); b.Size=UDim2.new(1,0,0,h or 32); b.BackgroundColor3=CARD; b.BackgroundTransparency=0.08
        b.BorderSizePixel=0; b.Font=Enum.Font.GothamBold; b.Text=text; b.TextColor3=WHITE; b.TextSize=10
        b.AutoButtonColor=true; corner(b,7); stroke(b,Color3.fromRGB(55,25,28),1)
        b.MouseButton1Click:Connect(cb); return b
    end
    local function toggle(text,cb)
        local on=false
        local b=btn(text.."  OFF",function()
            local wanted=not on
            local ok,err=pcall(cb,wanted)
            if ok then
                on=wanted; b.Text=text..(on and "  ON" or "  OFF")
                b.BackgroundColor3=on and Color3.fromRGB(18,95,165) or CARD
            else
                on=false; b.Text=text.."  OFF"; b.BackgroundColor3=CARD
                warn("[SAEGRR] ",err)
            end
        end)
        return b,function(v)
            on=v and true or false; b.Text=text..(on and "  ON" or "  OFF")
            b.BackgroundColor3=on and Color3.fromRGB(18,95,165) or CARD
        end
    end

    local status=label("READY",26,9,MUTED); status.TextWrapped=true
    local function say(t) status.Text=tostring(t) end

    -- ===== filter popups =====
    local selectedAreas,selectedRarities={},{}
    local areaRows=filter.areaOptions(); local rarityRows=filter.rarityOptions()
    local popup=nil

    local function closePopup() if popup then popup:Destroy(); popup=nil end end

    local function popupSelector(kind,rows,selected,onChanged)
        closePopup()
        popup=Instance.new("Frame",gui); popup.Name=kind.."Filter"; popup.Size=UDim2.fromOffset(285,360)
        -- Put the selector beside the main panel, like the screenshot.
        local x=main.AbsolutePosition.X+main.AbsoluteSize.X+12
        local y=main.AbsolutePosition.Y+30
        local vp=workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1280,720)
        if x+285>vp.X-8 then x=main.AbsolutePosition.X-297 end
        if y+360>vp.Y-8 then y=math.max(8,vp.Y-368) end
        popup.Position=UDim2.fromOffset(x,y); popup.BackgroundColor3=BG; popup.BackgroundTransparency=0.06; popup.BorderSizePixel=0; popup.ZIndex=100; corner(popup,12)
        stroke(popup,Color3.fromRGB(190,25,30),2)

        local t=Instance.new("TextLabel",popup); t.Size=UDim2.new(1,-58,0,34); t.Position=UDim2.fromOffset(12,7)
        t.BackgroundTransparency=1; t.Font=Enum.Font.GothamBold; t.Text=kind=="RARITY" and "RARITY FILTER" or "AREA FILTER"
        t.TextColor3=WHITE; t.TextSize=13; t.TextXAlignment=Enum.TextXAlignment.Left; t.ZIndex=101
        local c=Instance.new("TextButton",popup); c.Size=UDim2.fromOffset(30,26); c.Position=UDim2.new(1,-40,0,8)
        c.BackgroundTransparency=1; c.Text="×"; c.TextColor3=Color3.fromRGB(80,190,255); c.Font=Enum.Font.GothamBold; c.TextSize=17; c.ZIndex=101
        c.MouseButton1Click:Connect(closePopup)

        local sc=Instance.new("ScrollingFrame",popup); sc.Size=UDim2.new(1,-18,1,-58); sc.Position=UDim2.fromOffset(9,46)
        sc.BackgroundTransparency=1; sc.BorderSizePixel=0; sc.ScrollBarThickness=3; sc.ScrollBarImageColor3=RED; sc.ZIndex=101
        sc.AutomaticCanvasSize=Enum.AutomaticSize.Y; sc.ScrollingDirection=Enum.ScrollingDirection.Y
        local lay=Instance.new("UIListLayout",sc); lay.Padding=UDim.new(0,4)
        local p=Instance.new("UIPadding",sc); p.PaddingBottom=UDim.new(0,8)

        local function rowButton(id,text)
            local b=Instance.new("TextButton",sc); b.Size=UDim2.new(1,-2,0,32); b.BorderSizePixel=0
            b.Font=Enum.Font.Gotham; b.TextSize=10; b.TextColor3=WHITE; b.TextXAlignment=Enum.TextXAlignment.Left
            b.Text="   "..(selected[id] and "☑  " or "☐  ")..tostring(text)
            b.BackgroundColor3=selected[id] and Color3.fromRGB(20,85,150) or Color3.fromRGB(12,24,40); b.ZIndex=102; corner(b,6)
            b.MouseButton1Click:Connect(function()
                selected[id]=not selected[id]
                b.Text="   "..(selected[id] and "☑  " or "☐  ")..tostring(text)
                b.BackgroundColor3=selected[id] and Color3.fromRGB(20,85,150) or Color3.fromRGB(12,24,40)
                onChanged()
            end)
            return b
        end
        for _,r in ipairs(rows or {}) do rowButton(r.id,r.label) end
        local done=Instance.new("TextButton",popup); done.Size=UDim2.new(1,-18,0,42); done.Position=UDim2.new(0,9,1,-51)
        done.BackgroundColor3=Color3.fromRGB(20,110,210); done.BorderSizePixel=0; done.Text="DONE"; done.TextColor3=WHITE
        done.Font=Enum.Font.GothamBold; done.TextSize=12; done.ZIndex=103; corner(done,7); done.MouseButton1Click:Connect(closePopup)
    end

    label("EGG FILTERS",18,11,BLUE)
    local areaBtn=btn("AREA FILTER  •  ANY",function()
        popupSelector("AREA",areaRows,selectedAreas,function()
            local ids,n={},0; for id,v in pairs(selectedAreas) do if v then ids[#ids+1]=id; n=n+1 end end
            filter.setAreas(ids); areaBtn.Text=n==0 and "AREA FILTER  •  ANY" or ("AREA FILTER  •  "..n.." SELECTED")
        end)
    end)
    local rarityBtn=btn("RARITY FILTER  •  ANY",function()
        popupSelector("RARITY",rarityRows,selectedRarities,function()
            local ids,n={},0; for id,v in pairs(selectedRarities) do if v then ids[#ids+1]=id; n=n+1 end end
            filter.setRarities(ids); rarityBtn.Text=n==0 and "RARITY FILTER  •  ANY" or ("RARITY FILTER  •  "..n.." SELECTED")
        end)
    end)

    local target="Income"
    local targetBtn=btn("TARGET BY  •  INCOME",function()
        target=target=="Income" and "Weight" or "Income"; filter.setTargetBy(target); targetBtn.Text="TARGET BY  •  "..target:upper()
    end)

    -- Live egg list, similar to the screenshot's target list.
    label("AVAILABLE EGGS",18,11,BLUE)
    local eggListFrame=Instance.new("Frame",content); eggListFrame.Size=UDim2.new(1,0,0,168); eggListFrame.BackgroundTransparency=1
    local eggScroll=Instance.new("ScrollingFrame",eggListFrame); eggScroll.Size=UDim2.new(1,0,1,0); eggScroll.BackgroundColor3=Color3.fromRGB(10,20,34)
    eggScroll.BorderSizePixel=0; eggScroll.ScrollBarThickness=3; eggScroll.ScrollBarImageColor3=RED; eggScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; corner(eggScroll,7)
    local eggLayout=Instance.new("UIListLayout",eggScroll); eggLayout.Padding=UDim.new(0,4)
    local eggPad=Instance.new("UIPadding",eggScroll); eggPad.PaddingTop=UDim.new(0,5); eggPad.PaddingLeft=UDim.new(0,5); eggPad.PaddingRight=UDim.new(0,5); eggPad.PaddingBottom=UDim.new(0,5)
    ...
    local function refreshEggRows()
        for _,v in ipairs(eggScroll:GetChildren()) do if v:IsA("TextButton") or v:IsA("TextLabel") then v:Destroy() end end
        local data=eggs.list() or {}
        table.sort(data,function(a,b) return (tonumber(a.value) or 0)>(tonumber(b.value) or 0) end)
        local max=math.min(#data,20)
        if max==0 then
            local e=Instance.new("TextLabel",eggScroll); e.Size=UDim2.new(1,-4,0,30); e.BackgroundTransparency=1; e.Text="No takeable eggs found"; e.TextColor3=MUTED; e.Font=Enum.Font.Gotham; e.TextSize=9
        else
            for i=1,max do
                local e=data[i]
                local b=Instance.new("TextButton",eggScroll); b.Size=UDim2.new(1,-4,0,29); b.BackgroundColor3=Color3.fromRGB(12,28,48); b.BorderSizePixel=0
                b.TextXAlignment=Enum.TextXAlignment.Left; b.Font=Enum.Font.Gotham; b.TextSize=9; b.TextColor3=WHITE
                local rar=(e.rarity and e.rarity~="?") and tostring(e.rarity) or "Unknown"
                b.Text="  "..tostring(e.name or "Egg").."  ["..rar.."]  •  "..fmt(e.value).."/s"
                corner(b,5)
                b.MouseButton1Click:Connect(function()
                    say("Target preview: "..tostring(e.name).." ["..rar.."]")
                end)
            end
        end
    end
    ...
    task.spawn(function()
        while gui.Parent do
            pcall(refreshEggRows)
            task.wait(1.5)
        end
    end)

    btn("REFRESH EGGS",function()
        pcall(function() eggs.invalidate("SAEGRR UI refresh"); eggs.list({},true) end)
        refreshEggRows(); say("Egg list refreshed")
    end)

    label("TP SPEED",18,11,BLUE)
    local move = BX.require("features.movement")
    local tpSpeedBox=Instance.new("TextBox",content)
    tpSpeedBox.Size=UDim2.new(1,0,0,32); tpSpeedBox.BackgroundColor3=CARD; tpSpeedBox.BackgroundTransparency=0.08
    tpSpeedBox.BorderSizePixel=0; tpSpeedBox.ClearTextOnFocus=false; tpSpeedBox.Font=Enum.Font.GothamBold
    tpSpeedBox.Text=tostring(math.floor(move.speed and move.speed() or 1000))
    tpSpeedBox.PlaceholderText="40 - 1000 MAX"; tpSpeedBox.TextColor3=WHITE; tpSpeedBox.PlaceholderColor3=MUTED
    tpSpeedBox.TextSize=10; tpSpeedBox.TextXAlignment=Enum.TextXAlignment.Center; corner(tpSpeedBox,7); stroke(tpSpeedBox,BLUE,1)
    tpSpeedBox.FocusLost:Connect(function()
        local n=tonumber(tpSpeedBox.Text) or 1000
        n=math.clamp(math.floor(n),40,1000)
        tpSpeedBox.Text=tostring(n)
        pcall(function() move.setSpeed(n) end)
        say("TP Speed: "..n.." / 1000 MAX")
    end)

    label("AUTO STEAL",18,11,BLUE)
    local autoBtn,setAutoVisual=toggle("AUTO STEAL",function(on)
        if on and hold.isOn() then hold.setEnabled(false) end
        if on then
            auto.setOptions("farm",{pick=filter.pick,continuous=true})
            local ok,why=auto.setEnabled(true,"farm")
            if ok==false then error(why or "Auto Steal refused") end
            say("Auto Steal running with current filters")
        else
            auto.setEnabled(false,"farm"); say("Auto Steal stopped")
        end
    end)
    toggle("STAY ON TREADMILL",function(on)
        local ok,why=hold.setEnabled(on); if on and ok==false then error(why or "Treadmill unavailable") end
        say(on and "Stay On Treadmill: ON" or "Stay On Treadmill: OFF")
    end)

    label("ABYSS OVERLORD",18,11,BLUE)
    local bossStatus=label("Reading...",28,9,MUTED); bossStatus.TextWrapped=true
    btn("ENTER THE BOSS WORLD",function()
        local ok,msg=boss.enter(); say(msg); bossStatus.Text=tostring(msg)
    end)
    toggle("AUTO ENTER",function(on)
        if on and not boss.isOn() then boss.setEnabled(true) end
        boss.setAutoEnter(on); say(on and "Auto Enter: ON" or "Auto Enter: OFF")
    end)
    local fightBtn,setFightVisual=toggle("AUTO FIGHT",function(on)
        fight.setEnabled(on); say(on and "Auto Fight: ON" or "Auto Fight: OFF")
    end)
    local fightStatus=label("Auto fight: off",28,9,MUTED); fightStatus.TextWrapped=true

    task.spawn(function()
        while gui.Parent do
            pcall(function() local s=boss.status(); if s and s.body then bossStatus.Text=tostring(s.body) end end)
            pcall(function() local s=fight.status(); if s and s.body then fightStatus.Text=tostring(s.body) end end)
            task.wait(1)
        end
    end)
    pcall(function() auto.onStop(function(_,whose) if whose=="farm" then setAutoVisual(false); say("Auto Steal stopped") end end) end)

    -- Floating reopen button.
    local menu=Instance.new("TextButton",gui); menu.Size=UDim2.fromOffset(76,30); menu.Position=UDim2.new(0,12,.5,-15)
    menu.BackgroundColor3=BG; menu.BorderSizePixel=0; menu.Text="SAEGRR"; menu.TextColor3=WHITE; menu.Font=Enum.Font.GothamBold; menu.TextSize=10; corner(menu,7); stroke(menu,BLUE,2)
    menu.Active=true; menu.Draggable=true
    menu.MouseButton1Click:Connect(function() main.Visible=not main.Visible; if not main.Visible then closePopup() end end)

    -- Rotating accent, matching the hub's original visual identity.
    local g=Instance.new("UIGradient",mainStroke)
    g.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(5,30,65)),ColorSequenceKeypoint.new(.25,BLUE),ColorSequenceKeypoint.new(.5,WHITE),ColorSequenceKeypoint.new(.75,BLUE),ColorSequenceKeypoint.new(1,Color3.fromRGB(5,30,65))})
    task.spawn(function() while gui.Parent do g.Rotation=(g.Rotation+3)%360; task.wait() end end)
end
